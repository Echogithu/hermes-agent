...
def _print_progress(
    ...
    Format:
      [done/total  ✓passed ✗failed ⏱inflight] status path (N tests, Xs)
...
def main() -> int:
        ...
        try:
            ...
            with lock:
                ...
                _print_progress(
                    ...
                    repo_root, pass_count, fail_count, test_counts,
                )
            return
        with lock:
            done_count += 1
    ...
    print()
    print(f"=== Summary: {len(files)} files ({total_tests} tests) in {elapsed:.1f}s ({args.jobs} workers) ===")
