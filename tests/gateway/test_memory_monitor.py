...
def test_periodic_timer_fires(caplog):
    caplog.set_level(logging.INFO, logger="gateway.memory_monitor")
    # Short interval so we can observe periodic ticks inside the test budget.
    ...
    # Baseline and shutdown carry prefixes, so the strict "[MEMORY] rss="
    # filter only counts background ticks.  Poll until two ticks arrive
    # instead of relying on a fixed sleep that can be flaky on loaded hosts.
